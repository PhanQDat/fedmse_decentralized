...
class ClientTrainer(object):
    ...
    def validate_model(self, new_model_state, valid_loader, threshold=0.1):
        """
        Validate a new model state by comparing its validation loss with the current model's loss.

        Args:
            new_model_state (dict): State dictionary of the new model to validate.
            valid_loader (DataLoader): Validation data loader.
            threshold (float): Acceptable increase in loss (as a fraction of current loss).

        Returns:
            bool: True if the new model is accepted, False if rejected.
        """
        logging.info(f"Client {self.client_id} validating new model...")
        self.model.eval()
        current_loss = 0.0
        new_loss = 0.0
        device = self.device

        # Save current model state to restore if new model is rejected
        ...
        with torch.no_grad():
            for batch_input in valid_loader:
                if torch.isnan(batch_input[0]).any() or torch.isinf(batch_input[0]).any():
                    logging.warning(f"Invalid data detected in valid_loader for client {self.client_id}")
                    return False
                _, _, loss = self.model(batch_input[0].to(device))
                if torch.isnan(loss) or torch.isinf(loss):
                    logging.warning(f"Invalid loss detected for current model in client {self.client_id}")
                    return False
                current_loss += loss.item()
            current_loss /= len(valid_loader)

        # Load new model state and compute loss
        self.model.load_state_dict(new_model_state)
        with torch.no_grad():
            for batch_input in valid_loader:
                if torch.isnan(batch_input[0]).any() or torch.isinf(batch_input[0]).any():
                    logging.warning(f"Invalid data detected in valid_loader for client {self.client_id}")
                    self.model.load_state_dict(current_model_state)  # Restore current model
                    return False
                _, _, loss = self.model(batch_input[0].to(device))
                if torch.isnan(loss) or torch.isinf(loss):
                    logging.warning(f"Invalid loss detected for new model in client {self.client_id}")
                    self.model.load_state_dict(current_model_state)  # Restore current model
                    return False
                new_loss += loss.item()
        ...
        # Compare losses
        if new_loss <= current_loss * (1 + threshold):
            logging.info(f"Client {self.client_id} accepted new model. Current loss: {current_loss:.6f}, New loss: {new_loss:.6f}")
            return True
        else:
            logging.info(f"Client {self.client_id} rejected new model. Current loss: {current_loss:.6f}, New loss: {new_loss:.6f}")
            self.model.load_state_dict(current_model_state)  # Restore current model
            return False
